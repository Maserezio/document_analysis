...
class Su_Binarizator:
    ...
    def contrast_image_construction(self, input_image):
        ...
        for i in range(input_image.shape[0]):
            for j in range(input_image.shape[1]):
                ...
                cont_values[i, j] = (np.max(window) - np.min(window)) / (np.max(window) + np.min(window) + 1e-3)  
        
        return cont_values

    def high_contrast_pixel_detection(self, cont_values):
        cont_values[cont_values > 1] = 1
    ...
    def thresholding(self, otsu_image, input_image):
        ...
        for i in range(len(input_image)):
            for j in range(len(input_image[0])):
                ...
                E = np.delete(E, np.where(E == otsu_image[i, j])[0][0])
                I = np.delete(I, np.where(I == input_image[i, j])[0][0])

                Ne = np.sum(E == 255)

                E_mean = np.sum(I * (1 - E)) / Ne if Ne != 0 else 0
    ...
    def fit(self, input_images):
        ...
        for image in input_images:
            cont_values = self.contrast_image_construction(image)
            
            otsu_image = self.high_contrast_pixel_detection(cont_values)
            
            bin_image = self.thresholding(otsu_image, image)
